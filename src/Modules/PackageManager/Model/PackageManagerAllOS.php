...
class PackageManagerAllOS extends BaseLinuxApp
{
    ...
    public function setPackager($packagerName = null)
    {
        ...
        } elseif (isset($this->params["packagername"])) {
            $this->packagerName = $this->params["packagername"];
        } elseif (isset($this->params["packager-name"])) {
    ...
    public function setVersion($version = null)
    {
        ...
        } elseif (isset($this->params["version"])) {
    ...
    public function setVersionAccuracy($versionAccuracy = null)
    {
        ...
        } elseif (isset($this->params["version-accuracy"])) {
    ...
    public function setModule($moduleName = null)
    {
        ...
        } elseif (isset($moduleName) && is_object($moduleName)) {
            $this->moduleName = $moduleName->getModuleName();
        } elseif (isset($this->params["force-modulename"])) {
            $this->moduleName = $this->params["force-modulename"];
        } elseif (isset($this->params["force-module-name"])) {
    ...
    protected function installPackages()
    {
        ...
        if (!is_array($this->packageName)) {
            $this->packageName = array($this->packageName);
        }
        $returns = array() ;
        foreach ($this->packageName as $onePackage) {
            ...
            if ($result == true) {
                $this->setPackageStatusInCleovars($onePackage, true) ;
            } ;
            $returns[] = $result ;
        }
    ...
    {
        $packager = $this->getPackager() ;
        if ($packager->isInstalled($this->packageName)) {
            return true ;
        }
        return false;
    }
    ...
    public function packageIsRequired($packageName)
    {
        ...
        foreach ($modsRequiring as $modRequiring) {
            if ($modRequiring != $this->moduleName) {
                $finalModsRequiring[] = $modRequiring ;
            }
        }
    ...
    public function setPackageStatusInCleovars($packageName, $bool)
    {
        if ($bool == true) {
            if (is_array($packageName)) {
                foreach ($packageName as $onePackageName) {
                    $this->addPackageToCleovars($onePackageName) ;
                }
            } else {
                $this->addPackageToCleovars($packageName) ;
            }
        } else {
            if (is_array($packageName)) {
                foreach ($packageName as $onePackageName) {
                    $this->removePackageFromCleovars($onePackageName) ;
                }
            } else {
