...
class ApacheVHostEditorUbuntuLegacy extends Base
{
    ...
    {
        if (isset($this->params["vhe-docroot"])) {
            return $this->params["vhe-docroot"] ;
        }
    ...
    {
        if (isset($this->params["vhe-cluster-name"])) {
            return $this->params["vhe-cluster-name"] ;
        }
    ...
    protected function askForVHostDirectory()
    {
        ...
        if ($this->detectDebianApacheVHostFolderExistence()) {
            $question .= ' Found "/etc/apache2/sites-available" - Enter nothing to use this';
            if (isset($this->params["guess"])) {
    ...
    {
        if (isset($this->params["vhost"])) {
            return array($this->params["vhost"]) ;
        }
...
TEMPLATE5;
    ...
    protected function selectBalancerVHostTemplate()
    {
        if (isset($this->params["vhe-template"])) {
            $this->vHostTemplate = $this->params["vhe-template"] ;
            return ;
        } elseif (isset($this->params["vhe-default-template-name"])) {
