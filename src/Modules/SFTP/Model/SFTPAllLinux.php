...
class SFTPAllLinux extends Base
{
    ...
    {
        if ($this->askForSFTPExecute() != true) {
            return false;
        }
...
//                var_dump("en:", $allhe[0], $env["any-app"]["gen_env_name"]) ;
    ...
    protected function findTargetScopeString()
    {
        ...
        if (isset($this->params["hops"])) {
            ...
            } elseif (isset($this->params["hop-env-scope"]) && $this->params["hop-env-scope"] == "private") {
            ...
            if (isset($this->params["env-scope"]) && $this->params["env-scope"] == "public") {
                ...
                $target_scope_string = "target_public" ;
            } elseif (isset($this->params["env-scope"]) && $this->params["env-scope"] == "private") {
    ...
    protected function attemptSFTPConnection($server)
    {
        ...
        if (1 === 0) {
            ...
            if ($sftp->login($server["user"], $pword) == true) {
                return $sftp;
            }
            return null;
        } else {
...
QUESTION;
    ...
    protected function getSourceFilePath($flag = null)
    {
        ...
        if (isset($flag)) {
            $question = "Enter $flag source file path" ;
        } else {
            $question = "Enter source file path";
        }
        $input = self::askForInput($question) ;
    ...
    protected function getTargetFilePath($flag = null)
    {
        ...
        if (isset($flag)) {
            $question = "Enter $flag target file path" ;
        } else {
            $question = "Enter target file path";
        }
        $input = self::askForInput($question) ;
        return ($input=="") ? false : $input ;
    }
