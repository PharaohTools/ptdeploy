...
class VariableGroupsAllOS extends Base
{
    ...
    {
        $vg = null ;
        if (isset($this->params["vargroups"])) {
            $vg = $this->params["vargroups"] ;
        }
        if (isset($this->params["vars"])) {
            $vg = $this->params["vars"] ;
        }
        if (isset($this->params["variables"])) {
            $vg = $this->params["variables"] ;
        }
        if (!is_null($vg)) {
            return explode(",", $vg) ;
        }
    ...
    protected function findVariableFilePath($set)
    {
        if (file_exists($set)) {
            return $set ;
        } elseif (file_exists(getcwd().DS.$set)) {
            return getcwd().DS.$set ;
        }
        return null;
    }
}
