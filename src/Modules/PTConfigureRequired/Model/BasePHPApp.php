...
class BasePHPApp extends Base
{
    ...
    public function install()
    {
        ...
        if ($this->deleteProgramDataFolderAsRootIfExists() === false) {
            return false ;
        }
        if ($this->makeProgramDataFolderIfNeeded() === false) {
            return false ;
        }
        if ($this->doGitClone() === false) {
            return false ;
        }
        if ($this->copyFilesToProgramDataFolder() === false) {
            return false ;
        }
        ...
        if ($this->populateExecutorFile() === false) {
            return false ;
        }
        if ($this->saveExecutorFile() === false) {
            return false ;
        }
    ...
    public function unInstall($autoPilot = null)
    {
        ...
        if ($this->deleteExecutorIfExists() === false) {
            return false ;
        }
        if ($this->showCompletion() === false) {
            return false ;
        }
        return true ;
    }
    ...
    protected function askForProgramExecutorFolder()
    {
        ...
        } elseif (in_array(PHP_OS, array("Darwin"))) {
...
?>";
    ...
    protected function copyFilesToProgramDataFolder()
    {
        ...
        foreach ($this->fileSources as $fileSource) {
            $fs = "" ;
            if (isset($fileSource[3])&& $fileSource[3]==true) {
                $fs = DS.'.' ;
            }
    ...
    protected function doGitClone()
    {
        ...
        foreach ($this->fileSources as $fileSource) {
            ...
            if ($fileSource[2] != null) {
                $command .= '-b '.$fileSource[2].' ';
            }
