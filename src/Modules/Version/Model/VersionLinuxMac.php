    ...
    private function selectAppVersion()
    {
        ...
        if (count($otherResults)>0) {
            ...
            foreach ($otherResults as $result) {
                ...
                if (!is_dir($this->appRootDirectory.'/'.$result)) {
                    continue ;
                }
        ...
        if ($this->versionLimit != null && $this->versionLimit>0) {
            ...
            foreach ($allEntries as $currentKey => $oneEntry) {
                ...
                if (is_dir($fullDirPath) == null) {
                    unset($allEntries[$currentKey]);
                } // remove entry from array if not directory
                elseif ($project::checkIsPharaohProject($fullDirPath) == false) {
                    unset($allEntries[$currentKey]);
                } // remove entry from array if directory not a project
                elseif ($oneEntry=="." || $oneEntry=="..") {
                    unset($allEntries[$currentKey]);
                }// remove entry from array if its dot notation
                $i++;
            }
