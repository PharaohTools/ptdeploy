...
class ApacheControlAllLinux extends Base
{
    ...
    {
        if (!$this->askForApacheCtl("restart")) {
            return false;
        }
        $this->apacheCommand = $this->askForApacheCommand();
        $this->restartApache();
    ...
    {
        if (!$this->askForApacheCtl("reload")) {
            return false;
        }
        $this->apacheCommand = $this->askForApacheCommand();
        $this->reloadApache();
    ...
    {
        if (!in_array($type, array("start", "stop", "restart", "reload"))) {
            return false;
        }
    ...
    private function askForApacheCommand()
    {
        ...
        } elseif (in_array($linuxTypeFromConfig, array("debian", "redhat"))) {
