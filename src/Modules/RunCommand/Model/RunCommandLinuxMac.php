...
//        foreach ($commandRay as $command) { echo $command."\n" ; }
    ...
    public function askForUserName()
    {
        if (isset($this->params["run-as-user"])) {
            ...
            } elseif (isset($this->params["run-as-user"]) && strlen($this->params["run-as-user"])==0) {
                $this->runUser = null ;
            } else {
                $this->runUser = null ;
            }
    ...
    public function askForBackground()
    {
        ...
        } elseif (isset($this->params["background"]) && $this->params["background"]===true) {
            $this->background = true ;
        } elseif (isset($this->params["background"]) && strlen($this->params["background"])==0) {
            $this->background = null ;
        } elseif (isset($this->params["guess"]) && $this->params["guess"]==true) {
            $this->background = null ;
            return ;
        } else {
            $this->background = null;
